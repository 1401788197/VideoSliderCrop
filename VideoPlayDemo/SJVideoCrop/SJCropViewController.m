...
- (UIButton *)exportBtn {
	if (!_exportBtn) {
		_exportBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.width - 70, 100, 50, 20)];
		[_exportBtn setTitle:@"裁剪" forState:UIControlStateNormal];
...
/// 导出视频
- (void)exportClick {
	[ExportVideoUtil export:self.cropView.avAsset range:[self getTimeRange] complete:^(NSString * _Nonnull exportFilePath, NSError * _Nonnull error) {
	         NSLog(@"视频地址--%@",exportFilePath);
	 }];

}

- (CMTimeRange)getTimeRange {
	CMTime start = CMTimeMakeWithSeconds(_mediaConfig.startTime, self.palyView.player.currentTime.timescale);
	CMTime duration = CMTimeMakeWithSeconds(_mediaConfig.endTime - _mediaConfig.startTime, self.palyView.player.currentTime.timescale);
